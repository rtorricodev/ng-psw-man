...
export default class PasswordCardController {
  ...
  add(requrest: express.Request, response: express.Response): void {
    ...
    response.send({
      payload: passwordCard,
      message: "Password Card was successfully added.",
    });
  }
  ...
  update(request: express.Request, response: express.Response): void {
    ...
    const index = this.passwordCardList.findIndex(
      (passwordCard: PasswordCard): boolean => passwordCard.id.toString() === id
    );
    ...
    if (index > -1) {
      ...
      response.send({
        payload: passwordCard,
        message: `Password Card with id ${id} was updated.`,
      });
    } else {
  ...
  delete(resquest: express.Request, response: express.Response): void {
    ...
    if (index > -1) {
      ...
      response.send({
        message: `Password Card with id ${id} was deleted.`,
        error: false,
      });
    } else {
      response.send({
        message: `Password Card with id ${id} don't match any registerd Password Card.`,
        error: true,
      });
    }
  }
